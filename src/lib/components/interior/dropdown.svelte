...
<div bind:this={root} class="relative inline-block text-left {className}">
	...
	<AnimatePresence>
		{#if open}
			<motion.div initial={reducedMotion.current ? { opacity: 0 } : { opacity: 0, scale: 0.94, y: -8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={reducedMotion.current ? { opacity: 0 } : { opacity: 0, scale: 0.97, y: -6, transition: { duration: 0.12, ease: [0.4, 0, 1, 1] } }} transition={reducedMotion.current ? { duration: 0 } : { type: 'spring', stiffness: 620, damping: 38, mass: 0.6 }} style="transform-origin: top left" class="absolute left-0 top-[calc(100%+6px)] z-50 min-w-[224px] whitespace-nowrap rounded-[11px] border border-stone-200 bg-white p-[5px] shadow-[0_1px_2px_rgba(28,25,23,0.06),0_16px_36px_-18px_rgba(28,25,23,0.5)] dark:border-white/[0.16] dark:bg-[#1D1D1A]">
