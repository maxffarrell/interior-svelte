		...
		{#each rows as row (row.id)}
			<li aria-current={row.status === 'active' ? 'step' : undefined} class="flex h-7 items-center gap-2.5 px-1">
				<span class="relative grid size-4 shrink-0 place-items-center">
					{#if row.status === 'done'}
						<motion.span class="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-emerald-500/[0.14] text-emerald-600 dark:bg-emerald-400/[0.16] dark:text-emerald-400" initial={reducedMotion.current ? { opacity: 0 } : { opacity: 0, scale: 0.4 }} animate={{ opacity: 1, scale: 1 }} transition={reducedMotion.current ? { duration: 0 } : POP}><svg viewBox="0 0 256 256" width="11" height="11" fill="none" aria-hidden="true"><polyline points="216 72 104 184 48 128" stroke="currentColor" stroke-width="26" stroke-linecap="round" stroke-linejoin="round" /></svg></motion.span>
					{:else if row.status === 'error'}
						<motion.span class="col-start-1 row-start-1 grid size-4 place-items-center rounded-[5px] bg-red-500/[0.12] text-red-600 dark:bg-red-400/[0.14] dark:text-red-400" initial={reducedMotion.current ? { opacity: 0 } : { opacity: 0, scale: 0.4 }} animate={{ opacity: 1, scale: 1 }} transition={reducedMotion.current ? { duration: 0 } : POP}><svg viewBox="0 0 256 256" width="10" height="10" fill="none" aria-hidden="true"><path d="M200 56 56 200 M56 56l144 144" stroke="currentColor" stroke-width="26" stroke-linecap="round" /></svg></motion.span>
					{:else if row.status === 'active'}
						<span class="col-start-1 row-start-1 text-stone-500 dark:text-stone-400"><svg viewBox="0 0 16 16" class="size-3" aria-hidden="true" class:animate-spin={!reducedMotion.current}><circle cx="8" cy="8" r="6" fill="none" stroke="currentColor" stroke-opacity="0.25" stroke-width="2" /><path d="M8 2 a6 6 0 0 1 6 6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" /></svg></span>
					{:else}
						<span class="col-start-1 row-start-1 size-[5px] rounded-[2px] bg-stone-300 dark:bg-white/20"></span>
					{/if}
				</span>
				{#if row.status === 'active' && !reducedMotion.current}
					<span class="min-w-0 flex-1 truncate bg-[linear-gradient(90deg,#78716c_38%,#1c1917_50%,#78716c_62%)] bg-clip-text text-[12.5px] font-medium text-transparent [background-size:220%_100%] dark:bg-[linear-gradient(90deg,#a8a29e_38%,#fafaf9_50%,#a8a29e_62%)] animate-[task-shimmer_1.6s_linear_infinite]">{row.label}</span>
				{:else}
				...
				{/if}
				{#if row.meta}
					<span class="shrink-0 font-mono text-[10.5px] tabular-nums {row.status === 'done' ? 'text-stone-400 opacity-100 dark:text-stone-500' : 'opacity-0'}" aria-hidden={row.status !== 'done'}>{row.meta}</span>
				{/if}
			</li>
		{/each}
	</ol>
...
<style>
	...
	@media (prefers-reduced-motion: reduce) {
		:global(.animate-spin), :global(.animate-\[task-shimmer_1\.6s_linear_infinite\]) { animation: none; }
	}
</style>
